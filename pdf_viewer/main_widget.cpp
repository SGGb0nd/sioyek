...
void MainWidget::resizeEvent(QResizeEvent *resize_event) {
  ...
  if (main_document_view->get_is_auto_resize_mode()) {
    ...
    main_document_view->set_offset_y(main_window_height / 2 /
...
void MainWidget::set_overview_position(int page, float offset) {
  if (page >= 0) {
    auto abspos = main_document_view->get_document()->document_to_absolute_pos(
        {page, 0, offset});
    float page_height =
...
void MainWidget::mouseMoveEvent(QMouseEvent *mouse_event) {
  ...
  if (is_rotated()) {
    // we don't handle mouse events while document is rotated becausae proper
  ...
  if (overview_resize_data) {
    // if we are resizing overview page, set the selected side of the overview
    // window to the mosue position
    // float offset_diff_x = normal_x -
...
MainWidget::MainWidget(MainWidget *other)
    : MainWidget(other->mupdf_context, other->db_manager,
                 other->document_manager, other->config_manager,
                 other->command_manager, other->input_handler,
                 other->checksummer, other->should_quit) {}

MainWidget::MainWidget(fz_context *mupdf_context, DatabaseManager *db_manager,
                       DocumentManager *document_manager,
                       ConfigManager *config_manager,
                       CommandManager *command_manager,
                       InputHandler *input_handler,
                       CachedChecksummer *checksummer, bool *should_quit_ptr,
                       QWidget *parent)
    : QWidget(parent), mupdf_context(mupdf_context), db_manager(db_manager),
      document_manager(document_manager), config_manager(config_manager),
      input_handler(input_handler), checksummer(checksummer),
      should_quit(should_quit_ptr), command_manager(command_manager) {
...
#ifdef SIOYEK_QT6
    pdf_renderer =
        new PdfRenderer(4, should_quit_ptr, mupdf_context,
  ...
  opengl_widget = new PdfViewOpenGLWidget(main_document_view, pdf_renderer,
                                          config_manager, false, this);
  ...
  QObject::connect(pdf_renderer, &PdfRenderer::render_advance, this,
                   &MainWidget::invalidate_render);
  QObject::connect(pdf_renderer, &PdfRenderer::search_advance, this,
  ...
  connect(validation_interval_timer, &QTimer::timeout, [&, INTERVAL_TIME]() {
    ...
    if (main_document_view != nullptr) {
      ...
      if ((doc = main_document_view->get_document()) != nullptr) {
        ...
        // Wait until a safe amount of time has passed since the last time the
        // file was updated on the filesystem this is because LaTeX software
        // frequently puts PDF files in an invalid state while it is being made
        // in multiple passes.
  ...
  scroll_bar->connect(
      scroll_bar, &QScrollBar::actionTriggered, [this](int action) {
...
std::wstring MainWidget::get_status_string() {
  ...
  status_string.replace("%{current_page}",
                        QString::number(get_current_page_number() + 1));
  ...
  if (SHOW_DOCUMENT_NAME_IN_STATUSBAR) {
    ...
    if (file_name) {
      status_string.replace("%{document_name}",
  ...
  if (opengl_widget->get_is_searching(&progress)) {
    ...
    status_string.replace("%{search_results}",
                          " | showing result " + QString::number(result_index) +
  ...
  status_string.replace(
      "%{highlight}", " [ h" + QString::fromStdWString(highlight_select_char) +
  ...
  if (smooth_scroll_mode) {
    if (main_document_view_has_document()) {
      ...
      if (!opengl_widget->get_overview_page()) {
        ...
        main_document_view->set_offset_y(current_offset +
                                         smooth_scroll_speed * secs);
      } else {
      ...
      if (IGNORE_WHITESPACE_IN_PRESENTATION_MODE) {
        main_document_view->set_offset_y(
            main_document_view->get_document()->get_accum_page_height(
                current_page) +
            main_document_view->get_document()->get_page_height(current_page) /
                2);
      } else {
        main_document_view->set_offset_y(
            main_document_view->get_document()->get_accum_page_height(
            ...
            main_document_view->get_document()->get_page_height(current_page) /
                2 +
            static_cast<float>(get_status_bar_height() / 2 /
                               main_document_view->get_zoom_level()));
      }
      if (IGNORE_WHITESPACE_IN_PRESENTATION_MODE) {
...
void MainWidget::do_synctex_forward_search(const Path &pdf_file_path,
  ...
  synctex_scanner_p scanner =
      synctex_scanner_new_with_output_file(pdf_file_string.c_str(), nullptr, 1);

  int stat = synctex_display_query(scanner, latex_file_string.c_str(), line,
                                   column, 0);
  int target_page = -1;

  if (stat <= 0) {
    stat = synctex_display_query(
        scanner, latex_file_with_redundant_dot_string.c_str(), line, column, 0);
  }
...
void MainWidget::update_link_with_opened_book_state(
  ...
  db_manager->update_portal(link_owner->get_checksum(), new_state.offset_x,
                            new_state.offset_y, new_state.zoom_level,
                            lnk.src_offset_y);
...
void MainWidget::open_document_with_hash(const std::string &path,
...
void MainWidget::open_document(const Path &path, std::optional<float> offset_x,
                               std::optional<float> offset_y,
                               std::optional<float> zoom_level) {
  ...
  main_document_view->on_view_size_change(main_window_width,
                                          main_window_height);
  main_document_view->open_document(path.get_path(),
                                    &this->is_render_invalidated);
...
void MainWidget::open_document_at_location(const Path &path_, int page,
...
void MainWidget::open_document(const DocumentViewState &state) {
  open_document(state.document_path, state.book_state.offset_x,
...
void MainWidget::handle_command_types(std::unique_ptr<Command> new_command,
                                      int num_repeats) {
...
void MainWidget::key_event(bool released, QKeyEvent *kevent) {
  ...
  if (released == false) {
    ...
    std::vector<int> ignored_codes = {Qt::Key::Key_Shift, Qt::Key::Key_Control,
                                      Qt::Key::Key_Alt};
    if (std::find(ignored_codes.begin(), ignored_codes.end(), kevent->key()) !=
        ignored_codes.end()) {
      return;
    }
    if (is_waiting_for_symbol()) {
      ...
      char symb =
          get_symbol(kevent->key(), kevent->modifiers() & Qt::ShiftModifier,
    ...
    std::vector<std::unique_ptr<Command>> commands = input_handler->handle_key(
        kevent, kevent->modifiers() & Qt::ShiftModifier, is_control_pressed,
        kevent->modifiers() & Qt::AltModifier, &num_repeats);
...
void MainWidget::handle_right_click(WindowPos click_pos, bool down,
  ...
  if ((main_document_view->get_document() != nullptr) &&
    ...
    // after all) we can always use middle click to jump to a destination which
    // is probably what we need anyway
...
void MainWidget::handle_left_click(WindowPos click_pos, bool down,
  ...
  if (down == true) {
    ...
    if (opengl_widget->is_window_point_in_overview_border(normal_x, normal_y,
                                                          &border_index)) {
    ...
    if (opengl_widget->is_window_point_in_overview({normal_x, normal_y})) {
      ...
      opengl_widget->get_overview_offsets(&original_offset_x,
                                          &original_offset_y);
...
void MainWidget::prev_state() {
  ...
  if (current_history_index >= 0) {
    ...
    location, when going back, we update the link with the current location of
    document, therefore, we must check to see if a link is being edited and if
    so, we should update its destination position
    */
    if (link_to_edit) {
...
void MainWidget::set_main_document_view_state(
    DocumentViewState new_view_state) {
...
bool MainWidget::find_location_of_text_under_pointer(WindowPos pointer_pos,
                                                     int *out_page,
                                                     float *out_offset,
...
void MainWidget::mouseReleaseEvent(QMouseEvent *mevent) {
  ...
  if (mevent->button() == Qt::MouseButton::LeftButton) {
    if (is_shift_pressed) {
      ...
      auto commands =
          command_manager->create_macro_command("", CONTROL_CLICK_COMMAND);
      commands->run(this);
      ...
      handle_left_click({mevent->pos().x(), mevent->pos().y()}, false,
                        is_shift_pressed, is_control_pressed, is_alt_pressed);
      if (is_select_highlight_mode &&
  ...
  if (mevent->button() == Qt::MouseButton::RightButton) {
    if (is_shift_pressed) {
      auto commands =
          command_manager->create_macro_command("", SHIFT_RIGHT_CLICK_COMMAND);
      commands->run(this);
    } else if (is_control_pressed) {
      ...
      auto commands =
          command_manager->create_macro_command("", ALT_RIGHT_CLICK_COMMAND);
      commands->run(this);
    } else {
  ...
  if (mevent->button() == Qt::MouseButton::MiddleButton) {
    if (HIGHLIGHT_MIDDLE_CLICK &&
      ...
      command_manager->get_command_with_name("add_highlight_with_current_type")
          ->run(this);
      invalidate_render();
    } else {
...
void MainWidget::mousePressEvent(QMouseEvent *mevent) {
  ...
  if (mevent->button() == Qt::MouseButton::XButton1) {
    handle_command_types(command_manager->get_command_with_name("prev_state"),
                         0);
    invalidate_render();
  }

  if (mevent->button() == Qt::MouseButton::XButton2) {
    handle_command_types(command_manager->get_command_with_name("next_state"),
                         0);
    invalidate_render();
  }
}
    ...
    if (opengl_widget->is_window_point_in_overview({normal_x, normal_y})) {
      ...
      if (wevent->angleDelta().y() > 0) {
        ...
        if (is_visual_mark_mode) {
          command =
              command_manager->get_command_with_name("move_visual_mark_up");
        } else {
      ...
      if (wevent->angleDelta().y() < 0) {
        ...
        if (is_visual_mark_mode) {
          command =
              command_manager->get_command_with_name("move_visual_mark_down");
        } else {
          move_vertical(72.0f * vertical_move_amount * num_repeats_f);
          update_scrollbar();
          return;
...
void MainWidget::show_textbar(const std::wstring &command_name,
                              bool should_fill_with_selected_text) {
...
void MainWidget::smart_jump_under_pos(WindowPos pos) {
  ...
  // if overview page is open and we middle click on a paper name, search it in
  // a search engine
  if (opengl_widget->is_window_point_in_overview({normal_x, normal_y})) {
    auto [doc_page, doc_x, doc_y] =
        opengl_widget->window_pos_to_overview_pos({normal_x, normal_y});
  ...
  if (find_location_of_text_under_pointer(pos, &target_page,
                                          &target_y_offset)) {
    ...
    if (paper_name_on_pointer) {
      handle_paper_name_on_pointer(paper_name_on_pointer.value(),
                                   is_shift_pressed);
    }
  }
  ...
  // offset_x, offset_y); std::optional<std::wstring> reference_text_on_pointer
  // =
...
void MainWidget::visual_mark_under_pos(WindowPos pos) {
  ...
  if (document_pos.page != -1) {
    ...
    fz_pixmap *pixmap =
        main_document_view->get_document()->get_small_pixmap(document_pos.page);
    ...
    WindowPos window_pos = main_document_view->document_to_window_pos_in_pixels(
...
bool MainWidget::overview_under_pos(WindowPos pos) {
  ...
  if (find_location_of_text_under_pointer(pos, &autoreference_page,
                                          &autoreference_offset, true)) {
...
bool MainWidget::focus_on_visual_mark_pos(bool moving_down) {
  ...
  if ((moving_down && (ruler_window_rect.y0 < -thresh)) ||
    ...
    // main_document_view->get_zoom_level()) * VISUAL_MARK_NEXT_PAGE_FRACTION;
...
void MainWidget::execute_command(std::wstring command, std::wstring text,
  ...
  if (command_parts.size() > 0) {
    ...
    for (int i = 0; i < command_parts.size(); i++) {
      ...
      command_parts[i].replace("%4",
                               QString::number(get_current_page_number()));
      ...
      command_parts[i].replace("%{selected_text}",
                               QString::fromStdWString(selected_text));
      ...
      command_parts[i].replace("%{page_number}",
                               QString::number(get_current_page_number()));
      command_parts[i].replace("%{command_text}",
                               QString::fromStdWString(text));

      command_parts[i].replace("%{mouse_pos_window}",
                               QString::number(mouse_pos.x) + " " +
                                   QString::number(mouse_pos.y));
      // command_parts[i].replace("%{mouse_pos_window}", QString("%1
      // %2").arg(mouse_pos.x, mouse_pos.y));
      command_parts[i].replace("%{mouse_pos_document}",
      ...
      command_parts[i].replace("%{sioyek_path}",
      ...
      if (selected_line_text.size() > 0) {
        command_parts[i].replace("%6",
                                 QString::fromStdWString(selected_line_text));
        command_parts[i].replace("%{line_text}",
                                 QString::fromStdWString(selected_line_text));
      }
...
void MainWidget::handle_paper_name_on_pointer(std::wstring paper_name,
                                              bool is_shift_pressed) {
...
void MainWidget::get_window_params_for_one_window_mode(int *main_window_size,
...
void MainWidget::get_window_params_for_two_window_mode(
    int *main_window_size, int *main_window_move, int *helper_window_size,
...
void MainWidget::apply_window_params_for_two_window_mode() {
  ...
  get_window_params_for_two_window_mode(main_window_size, main_window_move,
                                        helper_window_size, helper_window_move);
...
void MainWidget::open_document(const std::wstring &doc_path, bool *invalid_flag,
                               bool load_prev_state,
                               std::optional<OpenedBookState> prev_state,
                               bool force_load_dimensions) {
  opengl_widget->clear_all_selections();

  main_document_view->open_document(doc_path, invalid_flag, load_prev_state,
                                    prev_state, force_load_dimensions);
...
std::optional<fz_irect>
MainWidget::get_tag_window_rect(std::string tag,
                                std::vector<fz_irect> *char_rects) {
...
void MainWidget::handle_link_click(const PdfLink &link) {
  ...
  // we usually just want to center the y offset and not the x offset (otherwise
  // for example a link at the right side of the screen will be centered,
  // causing most of screen state to be empty)
...
std::wstring MainWidget::get_serialized_configuration_string() {
  ...
  std::wstring overview_config_string =
      overview_config
          .arg(QString::number(overview_size[0]),
...
std::wstring MainWidget::get_window_configuration_string() {
  ...
  if ((helper_opengl_widget != nullptr) && helper_opengl_widget->isVisible()) {
    ...
    return (config_string_multi
                .arg(main_window_size_w, main_window_size_h, main_window_move_x,
                     main_window_move_y, helper_window_size_w,
                     helper_window_size_h, helper_window_move_x,
                     helper_window_move_y)
                .toStdWString());
  } else {
...
void MainWidget::focus_text(int page, const std::wstring &text) {
  ...
  for (int i = 0; i < line_texts.size(); i++) {
    ...
    int score = lcs(encoded_text.c_str(), encoded_line.c_str(),
                    encoded_text.size(), encoded_line.size());
...
#ifdef Q_OS_WIN
      // the path returned by synctex is formatted in unix style, for example it
      // is something like this in windows: d:/some/path/file.pdf
      // this doesn't work with Vimtex for some reason, so here we have to
      // convert the path separators to windows style and make sure the driver
      ...
      if (inverse_search_command.size() > 0) {
        ...
        QString command =
            QString::fromStdWString(inverse_search_command)
                .arg(new_path, line_string.c_str(), column_string.c_str());
#else
        QString command =
            QString::fromStdWString(inverse_search_command)
                .arg(file_name, line_string.c_str(), column_string.c_str());
#endif
...
void MainWidget::add_portal(std::wstring source_path, Portal new_link) {
  if (source_path == main_document_view->get_document()->get_path()) {
    ...
    db_manager->insert_portal(
        checksummer->get_checksum(source_path), new_link.dst.document_checksum,
        new_link.dst.book_state.offset_x, new_link.dst.book_state.offset_y,
...
void MainWidget::handle_keyboard_select(const std::wstring &text) {
  if (text[0] == '#') {
    ...
    // this is not something that the user should be able to do, but it's useful
    // for scripts.
    QStringList parts =
    ...
    if (parts.size() == 2) {
      ...
      if ((begin_parts.size() == 3) && (end_parts.size() == 3)) {
        ...
        DocumentPos begin_doc_pos = {begin_page_number, begin_offset_x,
                                     begin_offset_y};
        DocumentPos end_doc_pos = {end_page_number, end_offset_x, end_offset_y};

        WindowPos begin_window_pos =
            main_document_view->document_to_window_pos_in_pixels(begin_doc_pos);
        WindowPos end_window_pos =
            main_document_view->document_to_window_pos_in_pixels(end_doc_pos);
    ...
    if (parts.size() == 1) {
      ...
      if (schar_rects.size() > 1) {
        ...
        handle_left_click(
            {(srect.x0 + srect.x1) / 2 - 1, (srect.y0 + srect.y1) / 2}, true,
            false, false, false);
        handle_left_click({erect.x0, (erect.y0 + erect.y1) / 2}, false, false,
                          false, false);
        opengl_widget->set_should_highlight_words(false);
      }
    }
    if (parts.size() == 2) {
      ...
      if ((schar_rects.size() > 0) && (echar_rects.size() > 0)) {
        ...
        handle_left_click(
            {(srect.x0 + srect.x1) / 2 - 1, (srect.y0 + srect.y1) / 2}, true,
            false, false, false);
        handle_left_click({erect.x0 - w / 2, (erect.y0 + erect.y1) / 2}, false,
                          false, false, false);
        opengl_widget->set_should_highlight_words(false);
...
bool MainWidget::get_selected_rect_document(int &out_page, fz_rect &out_rect) {
  ...
  if (absrect) {
    ...
    DocumentPos bottom_right_document =
        main_document_view->get_document()->absolute_to_page_pos(bottom_right);
...
void MainWidget::goto_mark(char symbol) {
  if (symbol == '`' || symbol == '\'') {
    ...
    if (mark_vector.size() > 0) {
      assert(mark_vector.size() ==
             1); // we can not have more than one global mark with the same name
...
void MainWidget::perform_search(std::wstring text, bool is_regex) {
  ...
  opengl_widget->search_text(search_term, CASE_SENSITIVE_SEARCH, is_regex,
                             search_range);
}
...
void MainWidget::handle_add_highlight(char symbol) {
  if (main_document_view->selected_character_rects.size() > 0) {
    ...
    Highlight new_highlight =
        main_document_view->get_highlight_with_index(selected_highlight_index);
    main_document_view->delete_highlight_with_index(selected_highlight_index);
    main_document_view->add_highlight(new_highlight.selection_begin,
                                      new_highlight.selection_end, symbol);
...
void MainWidget::handle_goto_note(bool open = false) {
  std::vector<std::wstring> option_names;
  std::vector<std::wstring> option_location_strings;
  std::vector<Note> notes =
      main_document_view->get_document()->get_notes_sorted();

  int closest_note_index =
      main_document_view->get_document()->find_closest_note_index(
          notes, main_document_view->get_offset_y());

  for (auto note : notes) {
    option_names.push_back(note.description);
    auto [page, _, __] =
        main_document_view->get_document()->absolute_to_page_pos(
            note.selection_begin);
    option_location_strings.push_back(get_page_formatted_string(page + 1));
  }

  set_current_widget(new FilteredSelectTableWindowClass<Note>(
      option_names, option_location_strings, notes, closest_note_index,
      [&](Note *note) {
        if (note) {
          validate_render();
          push_state();
          main_document_view->set_offset_y(note->selection_begin.y);
        }
        if (open) {
          main_document_view->open_note(&note);
        }
      },
      this,
      [&](Highlight *note) {
        if (note) {
          main_document_view->delete_note_with_index(note->index);
        }
      }));
  current_widget->show();
}

void MainWidget::handle_goto_note_open() {
  MainWidget::handle_goto_note(true);
}
...
void MainWidget::handle_goto_toc() {
  ...
  if (main_document_view->get_document()->has_toc()) {
    if (FLAT_TABLE_OF_CONTENTS) {
      ...
      get_flat_toc(main_document_view->get_document()->get_toc(), flat_toc,
...
void MainWidget::handle_portal_to_link(const std::wstring &text) {
  ...
  if (selected_link_) {
    ...
    // AbsoluteDocumentPos abspos = doc()->document_to_absolute_pos(defpos[0],
    // true);
...
viod MainWidget::handle_open_note_under_cursor() {
  QPoint mouse_pos = mapFromGlobal(QCursor::pos());
  WindowPos window_pos = WindowPos{mouse_pos.x(), mouse_pos.y()};
  int sel_note = main_document_view->get_note_index_in_pos(window_pos);
  if (sel_note != -1) {
    main_document_view->open_note_with_index(sel_note);
  }
}
