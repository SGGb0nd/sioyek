...
public:
  ...
  virtual void set_text_requirement(std::wstring value) { this->text = value; }
};
...
class GotoNoteCommand : public Command {

  void perform(MainWidget *widget) { widget->handle_goto_note(); }

  bool pushes_state() { return true; }

  std::string get_name() { return "goto_note"; }
};

class GotoNoteOpenCommand : public Command {
  void perform(MainWidget *widget) { widget->handle_goto_note_open(); }

  bool pushes_state() { return true; }

  std::string get_name() { return "goto_note_open"; }
}
...
class AddNoteCommand : public SymbolCommand {

  void perform(MainWidget *widget) { widget->handle_add_note(symbol); }

  std::string get_name() { return "add_note"; }
}

class OpenNoteCommand : public Command {

  void perform(MainWidget *widget) { widget->handle_open_note(); }

  bool pushes_state() { return true; }

  std::string get_name() { return "open_note"; }
};
...
class CommandCommand : public Command {
  ...
  void perform(MainWidget *widget) {
    ...
    widget->set_current_widget(
        new CommandSelector(&widget->on_command_done, widget, command_names,
...
class DeleteNoteCommand : public Command {
  void perform(MainWidget *widget) {
    widget->main_document_view->delete_closest_note();
    widget->validate_render();
  }

  std::string get_name() { return "delete_note"; }
};
...
class DeleteNoteCommand : public Command {
  void perform(MainWidget *widget) {
    if (widget->selected_note_index != -1) {
      widget->main_document_view->delete_note_with_index(
          widget->selected_note_index);
      widget->selected_note_index = -1;
    }
    widget->validate_render();
  }
...
class OpenDocumentEmbeddedFromCurrentPathCommand : public Command {
  ...
  std::string get_name() { return "open_document_embedded_from_current_path"; }

  bool requires_document() { return false; }
};

class CopyCommand : public Command {
  void perform(MainWidget *widget) { copy_to_clipboard(widget->selected_text); }
  ...
  void perform(MainWidget *main_widget) {
    if (num_repeats) {
      main_widget->main_document_view->goto_page(
          num_repeats - 1 + main_widget->main_document_view->get_page_offset());
    } else {
...
public:
  ...
  virtual std::optional<Requirement> next_requirement(MainWidget *widget) {
    if (text.has_value()) {
      ...
      } else if ((widget->num_visible_links() < 10) &&
...
class KeysCommand : public Command {
  ...
  void perform(MainWidget *widget) { open_file(default_keys_path.get_path()); }
...
class GotoLeftCommand : public Command {
  ...
  void perform(MainWidget *widget) { widget->main_document_view->goto_left(); }
...
class GotoRightCommand : public Command {
  ...
  void perform(MainWidget *widget) { widget->main_document_view->goto_right(); }
...
class AddHighlightWithCurrentTypeCommand : public Command {
  void perform(MainWidget *widget) {
    if (widget->main_document_view->selected_character_rects.size() > 0) {
      widget->main_document_view->add_highlight(widget->selection_begin,
                                                widget->selection_end,
                                                widget->select_highlight_type);
...
class SetStatusStringCommand : public TextCommand {
  ...
  void perform(MainWidget *widget) { widget->set_status_message(text.value()); }
...
class PreviousPreviewCommand : public Command {
  void perform(MainWidget *widget) {
    if (widget->smart_view_candidates.size() > 0) {
      widget->index_into_candidates = mod(widget->index_into_candidates - 1,
                                          widget->smart_view_candidates.size());
      widget->set_overview_position(
          widget->smart_view_candidates[widget->index_into_candidates].page,
          widget->smart_view_candidates[widget->index_into_candidates].y);
...
private:
  ...
  Command *get_command() {
    if (actual_command) {
      ...
      actual_command =
          std::move(command_manager->get_command_with_name(command_name));
      if (!actual_command)
        return &noop;
...
public:
  ...
  MacroCommand(CommandManager *manager, std::string name_,
               std::wstring commands_) {
    ...
    for (int i = 0; i < parts.size(); i++) {
      if (parts.at(i).size() > 0) {
        commands.push_back(
            std::make_unique<LazyCommand>(manager, parts.at(i).toStdWString()));
      }
    }
  }
...
CommandManager::CommandManager(ConfigManager *config_manager) {
  ...
  for (auto [command_name_, command_value] : ADDITIONAL_COMMANDS) {
    ...
    new_commands[command_name] = [command_name, local_command_value]() {
      return std::make_unique<CustomCommand>(command_name, local_command_value);
    };
  }
  ...
  for (auto [command_name_, macro_value] : ADDITIONAL_MACROS) {
    ...
    new_commands[command_name] = [command_name, local_macro_value, this]() {
      return std::make_unique<MacroCommand>(this, command_name,
                                            local_macro_value);
    };
  }
  ...
  for (auto conf : configs) {
    ...
    // commands.push_back({ config_set_command_name, true, false , false, false,
    // true, {} });
...
CommandManager::get_command_with_name(std::string name) {
...
InputParseTreeNode *
parse_lines(InputParseTreeNode *root, const std::vector<std::wstring> &lines,
      ...
      if (!existing_node) {
        if ((tokens[i] != L"sym") && (tokens[i] != L"txt")) {
          if (parent_node->is_final) {
            std::wcout << L"Warning: key defined in " << command_file_names[j]
                       << L":" << command_line_numbers[j] << L" for "
                       << utf8_decode(command_names[j][0])
                       << L" is unreachable, shadowed by final key sequence "
                          L"defined in "
          ...
          if (tokens[i] == L"txt") {
            ...
            parent_node->is_final = true;
                 ...
                 (SHOULD_WARN_ABOUT_USER_KEY_OVERRIDE ||
                  (command_file_names[j].compare(
                      parent_node->defining_file_path)) == 0)) {
        if ((parent_node->name.size() == 0) ||
            parent_node->name[0].compare(command_names[j][0]) != 0) {

          std::wcout << L"Warning: key defined in "
                     << parent_node->defining_file_path << L":"
                     << parent_node->defining_file_line << L" overwritten by "
                     << command_file_names[j] << L":"
                     << command_line_numbers[j];
          if (parent_node->name.size() > 0) {
            std::wcout << L". Overriding command: " << line << L": replacing "
                       << utf8_decode(parent_node->name[0]) << L" with "
                       << utf8_decode(command_names[j][0]);
          }
          std::wcout << L"\n";
        }
...
InputParseTreeNode *
parse_lines(const std::vector<std::wstring> &lines,
            const std::vector<std::vector<std::string>> &command_names,
            const std::vector<std::wstring> &command_file_names,
            const std::vector<int> &command_line_numbers) {
  ...
  parse_lines(root, lines, command_names, command_file_names,
...
InputParseTreeNode *
parse_key_config_files(const Path &default_path,
  ...
  get_keys_file_lines(default_path, command_names, command_keys, command_files,
                      command_line_numbers);
  for (auto upath : user_paths) {
    get_keys_file_lines(upath, command_names, command_keys, command_files,
                        command_line_numbers);
  }

  return parse_lines(command_keys, command_names, command_files,
                     command_line_numbers);
}

InputHandler::InputHandler(const Path &default_path,
                           const std::vector<Path> &user_paths,
                           CommandManager *cm) {
  command_manager = cm;
...
void InputHandler::reload_config_files(
    ...
    const std::vector<Path> &user_config_paths) {
...
InputHandler::handle_key(QKeyEvent *key_event, bool shift_pressed,
                         bool control_pressed, bool alt_pressed,
                         int *num_repeats) {
  ...
  if (!USE_LEGACY_KEYBINDS) {
    ...
    if (((key_event->key() >= 'A') && (key_event->key() <= 'Z')) ||
        ((key_event->text().size() > 0) &&
         (std::find(special_texts.begin(), special_texts.end(),
                    key_event->text()) == special_texts.end()))) {
...
void InputHandler::delete_current_parse_tree(
    InputParseTreeNode *node_to_delete) {
  bool is_root = false;
...
std::string InputHandler::get_key_string_from_tree_node_sequence(
    const std::vector<InputParseTreeNode *> seq) const {
  std::string res;
...
std::unique_ptr<Command>
CommandManager::create_macro_command(std::string name,
                                     std::wstring macro_string) {
