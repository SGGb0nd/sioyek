...
DocumentView::DocumentView(fz_context *mupdf_context,
    ...
    : mupdf_context(mupdf_context), db_manager(db_manager),
      config_manager(config_manager), document_manager(document_manager),
...
DocumentView::DocumentView(fz_context *mupdf_context,
                           ...
                           DocumentManager *document_manager,
                           ConfigManager *config_manager,
                           CachedChecksummer *checksummer, bool *invalid_flag,
                           std::wstring path, int view_width, int view_height,
                           float offset_x, float offset_y)
    : DocumentView(mupdf_context, db_manager, document_manager, config_manager,
                   checksummer)
...
void DocumentView::set_offsets(float new_offset_x, float new_offset_y) {
  ...
  float max_y_offset = current_document->get_accum_page_height(num_pages - 1) +
...
void DocumentView::open_note_with_index(int index) {
  if (current_document) {
    open_note(current_document->get_notes()[index]);
  }
}
...
int DocumentView::get_note_index_in_pos(WindowPos window_pos) {
  auto [view_x, view_y] = window_to_absolute_document_pos(window_pos);

  fz_point pos = {view_x, view_y};

  if (current_document->can_use_notes()) {
    const std::vector<Note> &notes = current_document->get_notes();

    for (size_t i = 0; i < notes.size(); i++) {
      for (size_t j = 0; j < notes[i].highlight_rects.size(); j++) {
        if (fz_is_point_inside_rect(pos, notes[i].highlight_rects[j])) {
          return i;
        }
      }
    }
...
void DocumentView::add_highlight(AbsoluteDocumentPos selection_begin,
  ...
  if (current_document) {
    ...
    get_text_selection(selection_begin, selection_end, !EXACT_HIGHLIGHT_SELECT,
                       selected_characters, selected_text);
    merge_selected_character_rects(selected_characters, merged_characters);
    if (selected_text.size() > 0) {
      current_document->add_highlight(selected_text, merged_characters,
                                      selection_begin, selection_end, type);
    }
  }
}

void DocumentView::add_note(AbsoluteDocumentPos selection_begin,
                            AbsoluteDocumentPos selection_end, char type, ) {
  if (current_document) {
    std::vector<fz_rect> selected_characters;
    std::vector<fz_rect> merged_characters;
    std::wstring selected_text;
    std::wstring note_link;

    note_link = current_document->get_title() + L" " +
                current_document->get_vault_path();

    get_text_selection(selection_begin, selection_end, !EXACT_HIGHLIGHT_SELECT,
                       selected_characters, selected_text);
    merge_selected_character_rects(selected_characters, merged_characters);
    if (selected_text.size() > 0) {
      current_document->add_note(selected_text, merged_characters,
                                 selection_begin, selection_end, &note_link);
    }
  }
}

void DocumentView::open_note(Note note) {
  std::wstring vault_path = config_manager->get_config<std::wstring>(L"vault_path");
  std::wstring full_path = vault_path + L"/" + note.note_link;
...
void DocumentView::absolute_to_window_pos(float absolute_x, float absolute_y,
...
DocumentView::window_to_normalized_window_pos(WindowPos window_pos) {
  ...
  float normal_y =
      2 * (static_cast<float>(window_pos.y) - view_height / 2.0f) / view_height;
...
float DocumentView::set_zoom_level(float zl,
                                   bool should_exit_auto_resize_mode) {
...
float DocumentView::zoom_in_cursor(WindowPos mouse_pos, float zoom_factor) {
  ...
  move_absolute(-prev_doc_pos.x + new_doc_pos.x,
                prev_doc_pos.y - new_doc_pos.y);
...
void DocumentView::get_absolute_delta_from_doc_delta(float dx, float dy,
                                                     float *abs_dx,
                                                     float *abs_dy) {
...
void DocumentView::get_visible_pages(int window_height,
  ...
  current_document->get_visible_pages(window_y_range_begin, window_y_range_end,
                                      visible_pages);
...
void DocumentView::move_screens(int num_screens) {
  ...
  set_offset_y(get_offset_y() + num_screens * screen_height_in_doc_space *
...
void DocumentView::open_document(const std::wstring &doc_path,
                                 bool *invalid_flag, bool load_prev_state,
  ...
  if (prev_state) {
    ...
    if (checksum &&
      ...
      if (prev_state.size() > 1) {
        std::cerr << "more than one file with one path, this should not happen!"
                  << std::endl;
      }
    }
...
void DocumentView::fit_to_page_width(bool smart, bool ratio) {
  ...
  if (smart) {
    ...
    if (ratio) {
      virtual_view_width = static_cast<int>(static_cast<float>(view_width) *
...
void DocumentView::fit_to_page_height_width_minimum() {
  ...
  if (IGNORE_STATUSBAR_IN_PRESENTATION_MODE) {
    ...
    y_zoom_level = (static_cast<float>(view_height) - get_status_bar_height()) /
                   page_height;
  }
...
void DocumentView::persist() {
  if (!current_document)
    return;
  db_manager->update_book(current_document->get_checksum(), zoom_level,
                          offset_x, offset_y);
}
...
void DocumentView::get_page_chapter_index(int page,
...
void DocumentView::get_text_selection(
  ...
  if (current_document) {
    current_document->get_text_selection(selection_begin, selection_end,
                                         is_word_selection, selected_characters,
                                         selected_text);
  }
}
...
std::vector<DocumentPos> DocumentView::find_line_definitions() {
  // todo: remove duplicate code from this function, this just needs to find the
  // location of the
  ...
  if (line_index > 0) {
    ...
    if ((size_t)line_index < lines.size()) {
      ...
      std::optional<PdfLink> pdf_link = current_document->get_link_in_page_rect(
