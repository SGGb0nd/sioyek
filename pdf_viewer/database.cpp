...
static int null_callback(void *notused, int argc, char **argv,
                         char **col_name) {
  return 0;
}

static int opened_book_callback(void *res_vector, int argc, char **argv,
                                char **col_name) {
...
static int prev_doc_callback(void *res_vector, int argc, char **argv,
                             char **col_name) {
...
static int mark_select_callback(void *res_vector, int argc, char **argv,
                                char **col_name) {
...
static int global_mark_select_callback(void *res_vector, int argc, char **argv,
                                       char **col_name) {
...
static int global_bookmark_select_callback(void *res_vector, int argc,
...
static int global_highlight_select_callback(void *res_vector, int argc,
                                            char **argv, char **col_name) {
...
static int bookmark_select_callback(void *res_vector, int argc, char **argv,
                                    char **col_name) {
...
static int wstring_select_callback(void *res_vector, int argc, char **argv,
                                   char **col_name) {
...
static int string_select_callback(void *res_vector, int argc, char **argv,
                                  char **col_name) {
...
static int wstring_pair_select_callback(void *res_vector, int argc, char **argv,
                                        char **col_name) {
...
static int highlight_select_callback(void *res_vector, int argc, char **argv,
                                     char **col_name) {
...
static int link_select_callback(void *res_vector, int argc, char **argv,
                                char **col_name) {
...
bool DatabaseManager::open(const std::wstring &local_db_file_path,
  ...
  if (local_rc) {
    std::cerr << "could not create local database" << sqlite3_errmsg(local_db)
              << std::endl;
    local_db = nullptr;
    global_db = nullptr;
    return false;
...
bool DatabaseManager::create_opened_books_table() {
  ...
  int error_code = sqlite3_exec(global_db, create_opened_books_sql,
                                null_callback, 0, &error_message);
...
bool DatabaseManager::create_marks_table() {
  ...
  char *error_message = nullptr;
  int error_code = sqlite3_exec(global_db, create_marks_sql, null_callback, 0,
                                &error_message);
  return handle_error(error_code, error_message);
}
...
bool DatabaseManager::create_bookmarks_table() {
  ...
  char *error_message = nullptr;
  int error_code = sqlite3_exec(global_db, create_bookmarks_sql, null_callback,
                                0, &error_message);
  return handle_error(error_code, error_message);
}
...
bool DatabaseManager::create_highlights_table() {
  ...
  char *error_message = nullptr;
  int error_code = sqlite3_exec(global_db, create_highlights_sql, null_callback,
                                0, &error_message);
  return handle_error(error_code, error_message);
}

bool DatabaseManager::create_notes_table() {
  const char *create_notes_sql = "CREATE TABLE IF NOT EXISTS highlights ("
                                 "id INTEGER PRIMARY KEY AUTOINCREMENT,"
                                 "document_path TEXT,"
                                 "desc TEXT,"
                                 "type char,"
  ...
  char *error_message = nullptr;
  int error_code = sqlite3_exec(global_db, create_notes_sql, null_callback, 0,
                                &error_message);
  return handle_error(error_code, error_message);
}
...
bool DatabaseManager::create_document_hash_table() {
  ...
  char *error_message = nullptr;
  int error_code = sqlite3_exec(local_db, create_document_hash_sql,
                                null_callback, 0, &error_message);
  return handle_error(error_code, error_message);
}
...
bool DatabaseManager::insert_document_hash(const std::wstring &path,
  ...
  int delete_error_code =
      sqlite3_exec(local_db, utf8_encode(delete_ss.str()).c_str(),
                   null_callback, 0, &delete_error_message);
  handle_error(delete_error_code, delete_error_message);

  char *insert_error_message = nullptr;
  int insert_error_code =
      sqlite3_exec(local_db, utf8_encode(insert_ss.str()).c_str(),
                   null_callback, 0, &insert_error_message);
  return handle_error(insert_error_code, insert_error_message);
}

bool DatabaseManager::update_book(const std::string &path, float zoom_level,
                                  float offset_x, float offset_y) {
...
bool DatabaseManager::insert_mark(const std::string &document_path, char symbol,
                                  float offset_y) {
...
bool DatabaseManager::insert_bookmark(const std::string &document_path,
...
bool DatabaseManager::insert_highlight(const std::string &document_path,
                                       const std::wstring &desc, float begin_x,
                                       float begin_y, float end_x, float end_y,
                                       char type) {

  std::wstringstream ss;
  ss << "INSERT INTO highlights (document_path, desc, type, begin_x, begin_y, "
        "end_x, end_y) VALUES ('"
...
bool DatabaseManager::insert_note(const std::string &document_path,
                                  const std::wstring &desc, float begin_x,
                                  float begin_y, float end_x, float end_y,
                                  char type, const std::wstring &note_link) {

  std::wstringstream ss;
  ss << "INSERT INTO notes (document_path, desc, type, begin_x, begin_y, "
        "end_x, end_y, note_link) VALUES ('"
     << esc(document_path) << "', '" << esc(desc) << "', '" << type << "' , "
     << begin_x << " , " << begin_y << " , " << end_x << " , " << end_y << ", "
     << note_link << ");";
  char *error_message = nullptr;
...
bool DatabaseManager::insert_portal(const std::string &src_document_path,
                                    ...
                                    float dst_offset_x, float dst_offset_y,
                                    float dst_zoom_level, float src_offset_y) {
...
bool DatabaseManager::update_portal(const std::string &src_document_path,
                                    float dst_offset_x, float dst_offset_y,
                                    float dst_zoom_level, float src_offset_y) {
...
bool DatabaseManager::delete_link(const std::string &src_document_path,
                                  float src_offset_y) {
...
bool DatabaseManager::delete_bookmark(const std::string &src_document_path,
                                      float src_offset_y) {

  std::wstringstream ss;
  ss << "DELETE FROM bookmarks where document_path='" << esc(src_document_path)
     << "'AND abs(offset_y-(" << src_offset_y << ")) < 0.01;";
  char *error_message = nullptr;
...
bool DatabaseManager::delete_highlight(const std::string &src_document_path,
                                       float begin_x, float begin_y,
                                       float end_x, float end_y) {

  std::wstringstream ss;
  std::wstring threshold =
      QString::number(HIGHLIGHT_DELETE_THRESHOLD).toStdWString();
  ss << std::setprecision(10) << "DELETE FROM highlights where document_path='"
...
bool DatabaseManager::delete_note(const std::string &src_document_path,
                                  float begin_x, float begin_y, float end_x,
                                  float end_y) {

  std::wstringstream ss;
  std::wstring threshold =
      QString::number(HIGHLIGHT_DELETE_THRESHOLD).toStdWString();
  ss << std::setprecision(10) << "DELETE FROM notes where document_path='"
     << esc(src_document_path) << "'AND abs(begin_x-(" << begin_x << ")) < "
     << threshold << " AND abs(begin_y-(" << begin_y << ")) < " << threshold
     << " AND abs(end_x-(" << end_x << ")) < " << threshold
     << " AND abs(end_y-(" << end_y << ")) < " << threshold;
...
bool DatabaseManager::update_mark(const std::string &document_path, char symbol,
                                  float offset_y) {
...
bool DatabaseManager::select_opened_book(
  ...
  char *error_message = nullptr;
  int error_code =
      sqlite3_exec(global_db, utf8_encode(ss.str()).c_str(),
                   opened_book_callback, &out_result, &error_message);
  return handle_error(error_code, error_message);
}
...
//
//	return handle_error(
//		sqlite3_exec(db, utf8_encode(ss.str()).c_str(), null_callback,
...
bool DatabaseManager::select_opened_books_path_values(
  ...
  char *error_message = nullptr;
  int error_code = sqlite3_exec(global_db, utf8_encode(ss.str()).c_str(),
                                prev_doc_callback, &out_result, &error_message);
  return handle_error(error_code, error_message);
}
...
bool DatabaseManager::select_mark(const std::string &book_path,
                                  std::vector<Mark> &out_result) {
  std::wstringstream ss;
  ...
  int error_code =
      sqlite3_exec(global_db, utf8_encode(ss.str()).c_str(),
                   mark_select_callback, &out_result, &error_message);
  return handle_error(error_code, error_message);
}
...
bool DatabaseManager::select_global_mark(
  ...
  int error_code =
      sqlite3_exec(global_db, utf8_encode(ss.str()).c_str(),
                   global_mark_select_callback, &out_result, &error_message);
  return handle_error(error_code, error_message);
}

bool DatabaseManager::select_bookmark(const std::string &book_path,
                                      std::vector<BookMark> &out_result) {
  std::wstringstream ss;
  ...
  int error_code =
      sqlite3_exec(global_db, utf8_encode(ss.str()).c_str(),
                   bookmark_select_callback, &out_result, &error_message);
  return handle_error(error_code, error_message);
}

bool DatabaseManager::get_path_from_hash(const std::string &checksum,
  ...
  char *error_message = nullptr;
  int error_code =
      sqlite3_exec(local_db, utf8_encode(ss.str()).c_str(),
                   wstring_select_callback, &out_paths, &error_message);
  return handle_error(error_code, error_message);
}
...
bool DatabaseManager::get_hash_from_path(
  ...
  int error_code =
      sqlite3_exec(local_db, utf8_encode(ss.str()).c_str(),
                   wstring_select_callback, &out_checksum, &error_message);
  return handle_error(error_code, error_message);
}
...
bool DatabaseManager::get_prev_path_hash_pairs(
  ...
  int error_code =
      sqlite3_exec(local_db, utf8_encode(ss.str()).c_str(),
                   wstring_pair_select_callback, &out_pairs, &error_message);
  return handle_error(error_code, error_message);
}

bool DatabaseManager::select_highlight(const std::string &book_path,
  ...
  int error_code =
      sqlite3_exec(global_db, utf8_encode(ss.str()).c_str(),
                   highlight_select_callback, &out_result, &error_message);
  return handle_error(error_code, error_message);
}

bool DatabaseManager::select_note(const std::string &book_path,
                                  std::vector<Note> &out_result) {
  std::wstringstream ss;
  ss << "select desc, begin_x, begin_y, end_x, end_y, type, note_link from "
        "notes where document_path='"
     << esc(book_path) << "';";
...
bool DatabaseManager::select_highlight_with_type(
    const std::string &book_path, char type,
    std::vector<Highlight> &out_result) {
  std::wstringstream ss;
  ss << "select desc, begin_x, begin_y, end_x, end_y, type from highlights "
        "where document_path='"
...
bool DatabaseManager::select_note_with_type(const std::string &book_path,
                                            char type,
                                            std::vector<Note> &out_result) {
  std::wstringstream ss;
  ss << "select desc, begin_x, begin_y, end_x, end_y, type, note_link from "
...
bool DatabaseManager::global_select_highlight(
  ...
  std::wstringstream ss;
  ss << "select document_path, desc, type, begin_x, begin_y, end_x, end_y from "
        "highlights;";

  char *error_message = nullptr;
  int error_code = sqlite3_exec(global_db, utf8_encode(ss.str()).c_str(),
                                global_highlight_select_callback, &out_result,
                                &error_message);
  return handle_error(error_code, error_message);
}
...
bool DatabaseManager::global_select_bookmark(
  ...
  char *error_message = nullptr;
  int error_code = sqlite3_exec(global_db, utf8_encode(ss.str()).c_str(),
                                global_bookmark_select_callback, &out_result,
                                &error_message);
  return handle_error(error_code, error_message);
}

bool DatabaseManager::select_links(const std::string &src_document_path,
                                   std::vector<Portal> &out_result) {
  std::wstringstream ss;
  ...
  int error_code =
      sqlite3_exec(global_db, utf8_encode(ss.str()).c_str(),
                   link_select_callback, &out_result, &error_message);
  return handle_error(error_code, error_message);
}
...
bool update_string_value(sqlite3 *db, const std::wstring &table_name,
...
bool update_mark_path(sqlite3 *db, const std::wstring &path,
                      const std::wstring &new_path) {
  return update_string_value(db, L"marks", L"document_path", path, new_path);
}

bool update_opened_book_path(sqlite3 *db, const std::wstring &path,
                             const std::wstring &new_path) {
  return update_string_value(db, L"opened_books", L"path", path, new_path);
}

bool update_bookmark_path(sqlite3 *db, const std::wstring &path,
                          const std::wstring &new_path) {
  return update_string_value(db, L"bookmarks", L"document_path", path,
                             new_path);
}

bool update_highlight_path(sqlite3 *db, const std::wstring &path,
                           const std::wstring &new_path) {
  return update_string_value(db, L"highlights", L"document_path", path,
                             new_path);
}

bool update_portal_path(sqlite3 *db, const std::wstring &path,
                        const std::wstring &new_path) {
...
void DatabaseManager::split_database(const std::wstring &local_database_path,
  ...
  for (const auto &[hash, book_state] : opened_book_states) {
    update_book(hash, book_state.zoom_level, book_state.offset_x,
                book_state.offset_y);
  }
  ...
  for (const auto &[hash, highlight] : highlights) {
    insert_highlight(hash, highlight.description, highlight.selection_begin.x,
  ...
  for (const auto &[hash, portal] : portals) {
    insert_portal(hash, portal.dst.document_checksum,
...
template <typename T> QJsonArray export_array(std::vector<T> objects) {
  QJsonArray res;
...
void DatabaseManager::export_json(std::wstring json_file_path,
                                  CachedChecksummer *checksummer) {
...
template <typename T>
std::vector<T> get_new_elements(const std::vector<T> &prev_elements,
                                const std::vector<T> &new_elements) {
  std::vector<T> res;
...
void DatabaseManager::import_json(std::wstring json_file_path,
                                  CachedChecksummer *checksummer) {
  ...
  for (int i = 0; i < documents_json_array.size(); i++) {
    ...
    for (const auto &hl : new_highlights) {
      insert_highlight(checksum, hl.description, hl.selection_begin.x,
                       hl.selection_begin.y, hl.selection_end.x,
                       hl.selection_end.y, hl.type);
    }
    for (const auto &portal : new_portals) {
      insert_portal(checksum, portal.dst.document_checksum,
                    portal.dst.book_state.offset_x,
                    portal.dst.book_state.offset_y,
                    portal.dst.book_state.zoom_level, portal.src_offset_y);
    }
...
std::string create_select_query(
    std::string table_name, std::vector<std::string> selections,
    std::unordered_map<
        std::string, std::variant<std::wstring, std::string, int, char, float>>
        values) {
  std::wstringstream ss;
...
void DatabaseManager::ensure_database_compatibility(
  ...
  // if the database is still using absolute paths instead of checksums, update
  // all paths to checksums
  ...
  // if we are still using a single database file instead of separate local and
  // global database files, split the database.
