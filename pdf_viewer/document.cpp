...
void Document::fill_highlight_rects(fz_context *ctx, fz_document *doc_) {
  ...
  // merged highlight rects of individual characters in the highlighted area (we
  // merge the rects of characters in a single line which makes the result look
  // nicer)
  ...
  for (size_t i = 0; i < highlights.size(); i++) {
    ...
    get_text_selection(ctx, highlight.selection_begin, highlight.selection_end,
                       true, highlight_rects, highlight_text, doc_);
...
void Document::add_highlight(const std::wstring &desc,
                             ...
                             AbsoluteDocumentPos selection_end, char type) {
  if (type > 'z' || type < 'a') {
    type = 'a';
  }
  ...
  db_manager->insert_highlight(get_checksum(), desc, selection_begin.x,
                               selection_begin.y, selection_end.x,
                               selection_end.y, highlight.type);
}

void Document::add_note(const std::wstring &desc,
                        const std::vector<fz_rect> &note_rects,
                        AbsoluteDocumentPos selection_begin,
                        AbsoluteDocumentPos selection_end, char type,
                        const std::wstring &note_link) {
  if (type > 'z' || type < 'a') {
    type = 'a';
  }
  ...
  note.selection_begin = selection_begin;
  note.selection_end = selection_end;
  note.type = type;
  note.highlight_rects = note_rects;
  note.note_link = note_link;

  notes.push_back(note);
  db_manager->insert_note(get_checksum(), desc, selection_begin.x,
                          selection_begin.y, selection_end.x, selection_end.y,
                          note.type, note_link);
}
...
int Document::find_closest_bookmark_index(
    const std::vector<BookMark> &sorted_bookmarks, float to_offset_y) const {
...
int Document::find_closest_highlight_index(
    const std::vector<Highlight> &sorted_highlights, float to_offset_y) const {
...
int Document::find_closest_note_index(const std::vector<Note> &sorted_notes,
                                      float to_offset_y) const {
  int min_index = argminf<Note>(sorted_notes, [to_offset_y](Note note) {
    return abs(note.selection_begin.y - to_offset_y);
  });
...
void Document::delete_closest_bookmark(float to_y_offset) {
  ...
  if (closest_index > -1) {
    db_manager->delete_bookmark(get_checksum(),
                                bookmarks[closest_index].y_offset);
...
void Document::delete_note(Note note) {
  for (size_t i = (notes.size() - 1); i >= 0; i--) {
    if (notes[i] == note) {
      delete_note_with_index(i);
      return;
    }
  }
}

void Document::delete_note_with_index(int index) {
  Note note_to_delete = notes[index];

  db_manager->delete_note(get_checksum(), note_to_delete.selection_begin.x,
                          note_to_delete.selection_begin.y,
                          note_to_delete.selection_end.x,
                          note_to_delete.selection_end.y);
  notes.erase(notes.begin() + index);
}

std::optional<Portal> Document::find_closest_portal(float to_offset_y,
                                                    int *index) {
...
void Document::delete_closest_portal(float to_offset_y) {
  ...
  if (find_closest_portal(to_offset_y, &closest_index)) {
    db_manager->delete_link(get_checksum(),
                            portals[closest_index].src_offset_y);
...
const std::vector<Note> &Document::get_notes() const { return notes; }

const std::vector<Note> Document::get_notes_sorted() const {
  std::vector<Note> res = notes;
  std::sort(res.begin(), res.end(), [](const Note &lhs, const Note &rhs) {
    return lhs.selection_begin.y < rhs.selection_begin.y;
  });
  return res;
}

const std::vector<Note> Document::get_notes_of_type(char type) const {
  std::vector<Note> res;
...
Document::Document(fz_context *context, std::wstring file_name,
                   DatabaseManager *db, CachedChecksummer *checksummer)
    : db_manager(db), context(context), file_name(file_name),
      checksummer(checksummer), doc(nullptr) {
...
float Document::get_page_size_smart(bool width, int page_index,
                                    float *left_ratio, float *right_ratio,
                                    int *normal_page_width) {

  fz_pixmap *pixmap = get_small_pixmap(page_index);

  // project the small pixmap into a histogram (the `width` parameter determines
  // whether we do this horizontally or vertically)
...
void Document::convert_toc_tree(fz_outline *root,
                                std::vector<TocNode *> &output) {
...
void Document::get_visible_pages(float doc_y_range_begin, float doc_y_range_end,
  ...
  for (size_t i = 0; i < page_heights.size(); i++) {
    ...
    if (range_intersects(doc_y_range_begin, doc_y_range_end, page_begin,
                         page_end)) {
...
DocumentManager::DocumentManager(fz_context *mupdf_context, DatabaseManager *db,
                                 CachedChecksummer *checksummer)
    : mupdf_context(mupdf_context), db_manager(db), checksummer(checksummer) {
...
fz_stext_page *Document::get_stext_with_page_number(fz_context *ctx,
                                                    int page_number,
                                                    fz_document *doc_) {
...
fz_rect Document::absolute_to_page_rect(const fz_rect &absolute_rect,
                                        int *page) {
  int page_number = -1;
...
DocumentPos Document::absolute_to_page_pos(AbsoluteDocumentPos absp) {
  ...
  int i = (std::lower_bound(accum_page_heights.begin(),
                            accum_page_heights.end(), absp.y) -
...
int Document::get_offset_page_number(float y_offset) {
  ...
  auto it = std::lower_bound(accum_page_heights.begin(),
                             accum_page_heights.end(), y_offset);
...
void Document::index_document(bool *invalid_flag) {
  ...
  this->document_indexing_thread = std::thread([this, n, invalid_flag]() {
    ...
    fz_try(context_) {
      ...
      for (int i = 0; i < n; i++) {
        // when we close a document before its indexing is finished, we should
        // stop indexing as soon as posible
        if (!is_document_indexing_required) {
          break;
        }

        // we don't use get_stext_with_page_number here on purpose because it
        // would lead to many unnecessary allocations
        fz_stext_page *stext_page =
            fz_new_stext_page_from_page_number(context_, doc_, i, nullptr);

        std::vector<fz_stext_char *> flat_chars;
        get_flat_chars_from_stext_page(stext_page, flat_chars);

        if (SUPER_FAST_SEARCH) {
          flat_char_prism(flat_chars, i, local_super_fast_search_index,
...
std::optional<IndexedData>
Document::find_equation_with_string(std::wstring equation_name,
                                    int page_number) {
...
std::optional<std::wstring> Document::get_equation_text_at_position(
    const std::vector<fz_stext_char *> &flat_chars, float offset_x,
    float offset_y) {
...
std::optional<std::wstring> Document::get_regex_match_at_position(
    const std::wregex &regex, const std::vector<fz_stext_char *> &flat_chars,
  ...
  find_regex_matches_in_stext_page(flat_chars, regex, match_ranges,
                                   match_texts);

  for (size_t i = 0; i < match_ranges.size(); i++) {
    auto [start_index, end_index] = match_ranges[i];
    for (int index = start_index; index <= end_index; index++) {
      if (fz_contains_rect(fz_rect_from_quad(flat_chars[index]->quad),
                           selected_rect)) {
...
Document::find_generic_locations(const std::wstring &type,
  ...
  for (size_t i = 0; i < generic_indices.size(); i++) {
    ...
    if (parts.size() == 2) {
      ...
      if (current_name == name) {
        ...
        DocumentPos pos{generic_indices[i].page, 0,
                        generic_indices[i].y_offset};
  ...
  auto by_score = [](std::pair<int, DocumentPos> const &a,
                     std::pair<int, DocumentPos> const &b) {
...
void get_matches(std::wstring haystack, const std::wregex &reg,
...
Document::get_text_at_position(const std::vector<fz_stext_char *> &flat_chars,
                               float offset_x, float offset_y) {
...
fz_pixmap *Document::get_small_pixmap(int page) {
  ...
  fz_pixmap *res = fz_new_pixmap_from_page_number(context, doc, page, ctm,
...
void Document::get_text_selection(
    AbsoluteDocumentPos selection_begin, AbsoluteDocumentPos selection_end,
    bool
        is_word_selection, // when in word select mode, we select entire words
                           // even if the range only partially includes the word
    std::vector<fz_rect> &selected_characters, std::wstring &selected_text) {
  get_text_selection(context, selection_begin, selection_end, is_word_selection,
                     selected_characters, selected_text);
}
void Document::get_text_selection(fz_context *ctx,
                                  AbsoluteDocumentPos selection_begin,
  ...
  for (int i = page_begin; i <= page_end; i++) {
    ...
    if (i == page_begin) {
      char_begin = find_closest_char_to_document_point(flat_chars, page_point1,
                                                       &location_index1);
    }
    if (i == page_end) {
      char_end = find_closest_char_to_document_point(flat_chars, page_point2,
                                                     &location_index2);
    }
    ...
    for (auto current_char : flat_chars) {
      if (!is_word_selection) {
        ...
        if (word_selecting == true && is_separator(char_end, current_char) &&
            selecting == false) {
...
void Document::embed_annotations(std::wstring new_file_path) {
  ...
  for (auto highlight : doc_highlights) {
    ...
    get_text_selection(highlight.selection_begin, highlight.selection_end, true,
                       selected_characters, selected_text);
  ...
  for (auto bookmark : doc_bookmarks) {
    ...
    pdf_set_annot_contents(context, bookmark_annot,
                           encoded_bookmark_text.c_str());
...
std::optional<std::wstring>
Document::get_reference_text_at_position(int page, float offset_x,
                                         float offset_y) {
  fz_stext_page *stext_page = get_stext_with_page_number(page);
  std::vector<fz_stext_char *> flat_chars;
  get_flat_chars_from_stext_page(stext_page, flat_chars);
...
std::optional<std::wstring>
Document::get_equation_text_at_position(int page, float offset_x,
                                        float offset_y) {
  fz_stext_page *stext_page = get_stext_with_page_number(page);
  std::vector<fz_stext_char *> flat_chars;
  get_flat_chars_from_stext_page(stext_page, flat_chars);
...
std::optional<std::pair<std::wstring, std::wstring>>
Document::get_generic_link_name_at_position(int page, float offset_x,
                                            float offset_y) {
  fz_stext_page *stext_page = get_stext_with_page_number(page);
  std::vector<fz_stext_char *> flat_chars;
  get_flat_chars_from_stext_page(stext_page, flat_chars);
...
std::optional<std::wstring>
Document::get_regex_match_at_position(const std::wregex &regex, int page,
                                      float offset_x, float offset_y) {
  fz_stext_page *stext_page = get_stext_with_page_number(page);
  std::vector<fz_stext_char *> flat_chars;
  get_flat_chars_from_stext_page(stext_page, flat_chars);
...
std::vector<std::vector<fz_rect>> Document::get_page_flat_word_chars(int page) {
  // warning: this function should only be called after get_page_flat_words has
  // already cached the chars
...
std::optional<Highlight> Document::get_next_highlight(float abs_y, char type,
                                                      int offset) const {
...
std::optional<Highlight> Document::get_prev_highlight(float abs_y, char type,
                                                      int offset) const {
...
std::optional<PdfLink> Document::get_link_in_pos(int page, float doc_x,
                                                 float doc_y) {
  if (!doc)
    return {};
...
int Document::add_stext_page_to_created_toc(
    fz_stext_page *stext_page, int page_number,
  ...
  auto add_toc_node = [&](TocNode *node) {
    if (toc_node_stack.size() == 0) {
      ...
      while (
          (toc_node_stack.size() > 0) &&
          (!is_title_parent_of(toc_node_stack[toc_node_stack.size() - 1]->title,
                               node->title, &are_same)) &&
          (!are_same)) { // ignore items with the same name as current parent
                         // (happens when title of chapter is written on top of
                         // pages)
...
//		if (i > 0) {
//			//next_page =
// main_document_view->get_current_page_number() + 1; next_page =
// get_offset_page_number(absolute_y) + 1; 			if (next_page <
// num_pages()) { 				return
...
fz_rect Document::get_ith_next_line_from_absolute_y(int page, int line_index,
                                                    int i, bool cont,
                                                    int *out_index,
                                                    int *out_page) {
    ...
    *out_index = new_index;
    ...
    if (i > 0) {
      ...
      if (next_page < num_pages()) {
        return get_ith_next_line_from_absolute_y(next_page, 0, 0, false,
                                                 out_index, out_page);
      }
    } else {
      int next_page = page - 1;
      if (next_page >= 0) {
        return get_ith_next_line_from_absolute_y(next_page, -1, 0, false,
                                                 out_index, out_page);
      }
    }
    *out_page = page;
    ...
    if (stext_page && stext_page->first_block &&
      ...
      LL_ITER(block, stext_page->first_block) {
        if (block->type == FZ_STEXT_BLOCK_TEXT) {
          LL_ITER(line, block->u.t.first_line) { flat_lines.push_back(line); }
        }
      }
      ...
      for (size_t i = 0; i < line_locations_begins.size(); i++) {
        ...
        line_rect.y0 = document_to_absolute_y(
            page,
            static_cast<float>(line_locations_begins[i]) / SMALL_PIXMAP_SCALE);
        line_rect.y1 = document_to_absolute_y(
            page, static_cast<float>(line_locations[i]) / SMALL_PIXMAP_SCALE);
...
std::vector<SearchResult> Document::search_text(std::wstring query,
                                                bool case_sensitive,
                                                int begin_page, int min_page,
                                                int max_page) {
  ...
  auto it = std::search(super_fast_search_index.begin(),
                        super_fast_search_index.end(), searcher);
...
std::vector<SearchResult> Document::search_regex(std::wstring query,
                                                 bool case_sensitive,
                                                 int begin_page, int min_page,
                                                 int max_page) {
  std::vector<SearchResult> output;
  ...
  while (std::regex_search(search_start, super_fast_search_index.cend(), match,
                           regex)) {
    std::vector<fz_rect> match_rects;
    std::vector<fz_rect> compressed_match_rects;
