...
private:
  ...
  // reference with that name, but this is not necessarily true for the figures.
  ...
  Document(fz_context *context, std::wstring file_name,
           DatabaseManager *db_manager, CachedChecksummer *checksummer);
...
public:
  ...
  void add_highlight(const std::wstring &desc,
  ...
  void add_note(const std::wstring &desc,
                const std::vector<fz_rect> &highlight_rects,
                AbsoluteDocumentPos selection_begin,
                AbsoluteDocumentPos selection_end, char type,
                const std::wstring &note_text);
  void delete_note_with_index(int index);
  ...
  int find_closest_bookmark_index(const std::vector<BookMark> &sorted_bookmarks,
  ...
  int find_closest_note_index(const std::vector<Note> &sorted_notes,
                              float to_offset_y) const;

  std::optional<Portal> find_closest_portal(float to_offset_y,
                                            int *index = nullptr);
  ...
  const std::vector<Note> get_notes_of_type(char type) const;
  const std::vector<Note> get_notes_sorted() const;
  void open_note(Note *note);

  std::optional<Highlight> get_next_highlight(float abs_y, char type = 0,
                                              int offset = 0) const;
  std::optional<Highlight> get_prev_highlight(float abs_y, char type = 0,
                                              int offset = 0) const;
  ...
  bool open(bool *invalid_flag, bool force_load_dimensions = false,
            std::string password = "", bool temp = false);
  ...
  float get_page_size_smart(bool width, int page_index, float *left_ratio,
                            float *right_ratio, int *normal_page_width);
  float get_accum_page_height(int page_index);
  void rotate();
  void get_visible_pages(float doc_y_range_begin, float doc_y_range_end,
  ...
  get_text_at_position(const std::vector<fz_stext_char *> &flat_chars,
                       float offset_x, float offset_y);
  std::optional<std::wstring>
  get_reference_text_at_position(const std::vector<fz_stext_char *> &flat_chars,
                                 float offset_x, float offset_y);
  std::optional<std::wstring>
  get_paper_name_at_position(const std::vector<fz_stext_char *> &flat_chars,
                             float offset_x, float offset_y);
  std::optional<std::wstring>
  get_equation_text_at_position(const std::vector<fz_stext_char *> &flat_chars,
                                float offset_x, float offset_y);
  ...
  get_generic_link_name_at_position(
      ...
      float offset_y);
  std::optional<std::wstring>
  get_regex_match_at_position(const std::wregex &regex,
                              const std::vector<fz_stext_char *> &flat_chars,
                              float offset_x, float offset_y);
  std::optional<std::wstring> get_text_at_position(int page, float offset_x,
                                                   float offset_y);
  std::optional<std::wstring>
  ...
  std::optional<std::wstring>
  get_regex_match_at_position(const std::wregex &regex, int page,
                              float offset_x, float offset_y);
  std::vector<DocumentPos> find_generic_locations(const std::wstring &type,
  ...
  void get_text_selection(
      ...
      bool is_word_selection, // when in word select mode, we select entire
                              // words even if the range only partially includes
                              // the word
  ...
  int add_stext_page_to_created_toc(fz_stext_page *stext_page, int page_number,
  ...
  AbsoluteDocumentPos document_to_absolute_pos(DocumentPos,
                                               bool center_mid = false);
  fz_rect document_to_absolute_rect(int page, fz_rect doc_rect,
                                    bool center_mid = false);

  // void get_ith_next_line_from_absolute_y(float absolute_y, int i, bool cont,
  // float* out_begin, float* out_end);
  fz_rect get_ith_next_line_from_absolute_y(int page, int line_index, int i,
                                            bool cont, int *out_index,
                                            int *out_page);
  const std::vector<fz_rect> &
  get_page_lines(int page, std::vector<std::wstring> *line_texts = nullptr);

  bool is_super_fast_index_ready();
  std::vector<SearchResult> search_text(std::wstring query, bool case_sensitive,
                                        int begin_page, int min_page,
                                        int max_page);
  std::vector<SearchResult> search_regex(std::wstring query,
                                         bool case_sensitive, int begin_page,
                                         int min_page, int max_page);
  float max_y_offset();
...
public:
  DocumentManager(fz_context *mupdf_context, DatabaseManager *db_manager,
                  CachedChecksummer *checksummer);
